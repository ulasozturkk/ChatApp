...
namespace ChatApp.Utils.Security.Services.Concrete; 
public class SecurityService : ISecurityService {
  public string HashCreate(string value, string salt) {
    if (value != null && salt != null) {
      var valueBytes = KeyDerivation.Pbkdf2(value,
  ...
  public string HashPassword(string Password, string Username, string Salt) {
    return HashCreate(Password + Username, Salt);
  }

  public string EncryptToAES(string plainText, string keyString) {
    var Key = Encoding.UTF8.GetBytes(keyString.ToMD5());//16 bit or 32 bit key string
    var IV = Encoding.UTF8.GetBytes(keyString.ToMD5().Substring(0, 16));
